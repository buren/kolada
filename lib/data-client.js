...
class DataClient extends ApiClient {
  ...
  municipalities(options) {
    if (!options) return this.fetch(this.allDataUrl('municipality'));
  ...
  ou(options) {
    if (!options) return this.fetch(this.allDataUrl('ou'));
  ...
  allDataUrl(option) {
    if (option === 'ou') {
      return `${this.baseURL}/oudata/all`;
    } else if (option === 'municipality' || option === 'municipalities') {
      return `${this.baseURL}/data/all`;
    }

    throw new Error('Unknown value for "all" option - known are municipality and ou.')
  }
}
